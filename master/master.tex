...
A set of two kernels and three functions will be used giving a total of six tests.
The two kernels are $e^{-t}$ and $\cosh(t)$. The solutions for these kernels given
that $f(0) = 0$ is given by \cite{handbook} as
\begin{align}
\label{kernel1_soln}  e^{-t} &\implies f'(t) + f(t) \\
\label{kernel2_soln} \cosh(t) &\implies f'(t) - \int_0^t f(s) \; ds
\end{align}
The three functions we will use for testing will be $te^{-t}$, $t^3$, and $\sin t$.
\begin{table}[H]
